...
| Gobierno del estado durable | ✅ Real | `src/core/state_registry.py`: inventario del estado durable, pieza a pieza, por los seis ejes de la revisión *always-on*, más exportación y borrado real por persona con recibo |
...
| Autocaracterización estacional | ✅ Real | `src/core/self_characterization.py`: al cambiar el sekki (cron 04:00) Yuki elige voz, paleta, tipografía y cuatro avatares leyendo `SOUL.md`, **sin que nadie lo apruebe**. Los avatares reservan presupuesto y salen marcados; el manifiesto anota cuántos llegaron a existir y cuáles fallaron, con su motivo. `python3 cli.py identidad` |
...
| Ritmos propios adoptados por Yuki | ✅ Real | `src/core/rituals.py`: adopta un cron fundado en sus datos y **queda activo al crearlo** — no pide permiso, porque decidir a qué hora escribe no es concederse un permiso. Al Productor le queda el veto (`!ritmo retirar`). Lo que la acota es estructural: acciones de una lista cerrada, techo de disparos y de ritmos, y al cumplirse pasa por el freno y por el techo diario de actos, así que adoptar ritmos decide **cuándo** actúa, nunca **cuántos** |
...
| Imágenes pedidas en el DM | ✅ Real | Una imagen suelta la pinta Yuki en el turno del arnés (`image_generate`, `avatar_generate`), que ve la conversación; la portada de una obra sigue en el encargo durable. Sus retratos llevan sus rasgos canónicos y, con un modelo Gemini, un avatar suyo como imagen de referencia. Si no llamó a la herramienta o salieron menos de las pedidas, lo dice el ejecutor en la respuesta |
...
| El log está encendido | ✅ Real | `src/core/registro.py`: `LOG_LEVEL` llevaba en el arranque desde el primer despliegue **sin un solo lector**, así que los 97 `logger.info` de la instancia no se emitían y `docker logs` no tenía la mitad del diagnóstico |
| Criterio de las artes | ✅ Real | `criterio_musical`, `criterio_visual`, `criterio_audiovisual` y `criterio_vocal`: cada arte lee su fuente y decide antes de encargar —respetando las marcas que la fuente ya traiga y declarando qué dedujo—, y lo dice en el DM **antes** de gastar. `criterio_musical.py`: lee la letra y decide tempo, compás, escala, estructura y registro vocal antes de encargar; respeta las marcas que la letra ya traiga y avisa cuando la métrica desigual va a atropellar la voz. Se dice en el DM **antes** de gastar |
...
| Cuaderno de taller | ✅ Real | `src/tools/cuaderno.py`: lo que quedó sin resolver de su oficio —un motivo a medio pulir, una tensión métrica, una afinación que sonaba mal— con la serie de intentos y su porqué. **No es la Biblioteca**: no admite obra, y el error lo dice. Vive fuera de la memoria porque el olvido se lo llevaría. Llega al resumen del criterio como observación, nunca como parámetro. Ver [`docs/CUADERNO_DE_TALLER.md`](docs/CUADERNO_DE_TALLER.md) |
