...
class SalonHTTPHandler(BaseHTTPRequestHandler):
    ...
    def _metricas(self) -> str:
        """
        El texto de `/metrics`. Lo construye `metricas.exposicion()`, que no es
        HTTP y por eso vive aparte; aquí queda el nombre por el que ya se pide.
        """
        return metricas.exposicion()
